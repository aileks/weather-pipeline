...
- Nulls appear index-aligned in variable arrays. Nulls are kept as nulls (rows are not dropped); downstream tests bound how many are acceptable.
...
3. **Land.** Write the parsed rows **plus operational metadata** as a new immutable snapshot (next section).
...
- The city list (source of truth: the dbt seed described in [Transformation](transformation.md#dim_location-the-cities-seed), the ingestion config and the seed are generated from one canonical list, so they cannot drift)
...
└── year=2026/
    └── month=08/
        └── day=15/
            ├── ingested_at=2026-08-16T06-00-05Z.parquet   ← first successful fetch
            └── ingested_at=2026-08-23T06-00-04Z.parquet   ← reconciliation re-fetch
```

Hive-style date directories (year/month/day) plus one Parquet file per fetch, named by its ingestion timestamp (filesystem-safe: colons become hyphens). Every successful materialization of a partition appends exactly one snapshot. **Snapshots are never modified, never renamed, never deleted** (retention is unlimited; pruning is a documented limitation, [Design Decisions](design-decisions.md#limitations)).

### Why versioned rather than "one file per day"

A single overwriteable file per day cannot be both immutable and reconcilable: re-fetching a revised day would have to destroy the record of what was originally received. Versioned snapshots resolve the conflict, reconciliation *adds* `ingested_at=2026-08-23T06-00-04Z.parquet` and re-points the derivation, leaving history intact. This was the one necessary architecture correction of the design review; full reasoning in [Design Decisions](design-decisions.md).

### Write path and atomicity

A snapshot is written to a temporary file in the partition directory, then atomically renamed into place (`os.replace`). A run that crashes mid-write leaves at worst a stray temp file (ignored by snapshot selection), never a partial snapshot that looks valid. The ingestion timestamp in the filename comes from the materialization's run time, making snapshot identity deterministic per run and unique per fetch.
...
| `ingested_at_utc` | TIMESTAMPTZ | When this fetch happened |
| `source_url` | VARCHAR | The full request URL, the snapshot's provenance |

### Latest-snapshot selection

For a partition day, the **latest snapshot** is the file with the maximum `ingested_at` in its directory. Selection is a directory listing plus filename sort, no catalog, no database, no state outside the filesystem. Because writes are atomic and names are unique per run, listing can never observe a torn state.
...
- **Idempotent by construction.** Re-materializing a partition rewrites the same slice; row counts cannot drift, duplicates cannot accumulate, and a failed derive leaves the previous state (both statements run in one transaction).
- **Replayable without refetch.** The whole table can be rebuilt from the landing zone alone, the recovery procedure in [Operations Runbook](operations-runbook.md#rebuilding-the-warehouse-from-the-landing-zone) is exactly this derivation applied to every partition.
