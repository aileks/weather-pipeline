...
- **Transport failures** (timeouts, connection errors, HTTP 429/5xx) raise a retryable error and are retried up to 3 times with a 30-second wait between attempts. Control is explicit: the client raises Dagster's retry-requested exception for these only.
...
The mitigation is declarative, split between code and instance configuration. Every warehouse writer (the ingestion asset and both dbt asset groups) is assigned in code to the `warehouse` concurrency pool; instance configuration (`DAGSTER_HOME/dagster.yaml`) declares pool granularity `run` and a default limit of one, so at most one run touching a writer executes at a time. An explicit limit can also be set per pool with `dagster instance concurrency set warehouse 1`. Runs additionally carry the `warehouse=duckdb` tag for identification in the UI. Throughput is irrelevant at this scale (a partition materializes in seconds). This is a deliberate tradeoff of the DuckDB decision; the escape hatches (separate raw/serve files, or a client-server engine) are production-path options in [Design Decisions](design-decisions.md#moving-to-production).
